...
func TestUseCounter_Increment(t *testing.T) {
	...
	uc := NewUseCounter(sqlDB, 50*time.Millisecond)

	// Increment multiple times from several goroutines.
	for i := 0; i < 5; i++ {
...
func TestUseCounter_Shutdown_Flushes(t *testing.T) {
	...
	uc := NewUseCounter(sqlDB, 10*time.Minute)
	uc.Increment(link.ID)
	uc.Increment(link.ID)
	uc.Increment(link.ID)
...
func TestUseCounter_Ticker_Flush(t *testing.T) {
	...
	uc := NewUseCounter(sqlDB, 20*time.Millisecond)
	uc.Increment(link.ID)
	uc.Increment(link.ID)
